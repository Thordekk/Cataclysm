...
void BattlegroundWS::AddPlayer(Player* player)
{
    ...
    PlayerScores[player->GetGUID().GetCounter()] = new BattlegroundWGScore(player->GetGUID(), player->GetBGTeam());
}
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    UpdateFlagState(player->GetTeam(), BG_WS_FLAG_STATE_WAIT_RESPAWN);                  // flag state none
...
void BattlegroundWS::EventPlayerDroppedFlag(Player* player)
{
    ...
    if (set)
    {
        ...
        UpdateFlagState(player->GetTeam(), BG_WS_FLAG_STATE_ON_GROUND);
